...
# Auto-reset package-lock.json to a clean git state before pulling.
# It is auto-generated and regenerated by npm install, so local drift is never
# worth keeping. This also clears index quirks (skip-worktree / assume-unchanged)
# that otherwise make `git pull` abort with "Your local changes to the following
# files would be overwritten by merge: package-lock.json" even after a
# `git reset --hard` (the flags live in the index, not the commits).
if git ls-files package-lock.json >/dev/null 2>&1 || [ -f package-lock.json ]; then
  ...
  if [ -n "$(git status --porcelain -- package-lock.json 2>/dev/null)" ] || git check-ignore -q package-lock.json 2>/dev/null; then
    warn "package-lock.json is dirty or untracked. Discarding local copy (npm install will regenerate)..."
...
echo "  3. Open /_rv_preview in browser → enter credentials → test site"
...
echo ""
echo "  ─── Maintenance Helpers ───"
echo "  Enable:   MAINTENANCE_PASS=secret bash scripts/maintenance-on.sh"
echo "  Change:   MAINTENANCE_PASS=newpass bash scripts/maintenance-on.sh"
echo "  Disable:  bash scripts/maintenance-off.sh     # no password needed"
echo ""
echo "  ─── Handy Server Commands ───"
echo "  PM2:"
echo "    pm2 logs $PM2_PROCESS_NAME       # View live logs"
echo "    pm2 restart $PM2_PROCESS_NAME    # Restart server"
echo "    pm2 stop $PM2_PROCESS_NAME       # Stop server"
